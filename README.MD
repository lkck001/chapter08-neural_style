...
这是《深度学习框架PyTorch：入门与实践》第八章的配套代码，实现了快速风格迁移。
...
本试验使用的数据来自[COCO](http://cocodataset.org/#download)，点[此](http://images.cocodataset.org/zips/train2014.zip)下载（文件很大，14G）。 你也可以使用其它数据，比如ImageNet。请尽量保证数据的多样性，**不建议**使用单一种类的数据集，比如LSUN或者人脸识别数据集之类的。

请先解压 `./data/coco.zip` ，确保所有的图片保存于`data/coco/`文件夹下,形如：
```Bash
data
 └─ coco
...
读者可以[点此](https://yun.sfo2.digitaloceanspaces.com/pytorch_book/pytorch_book/transformer.pth)下载预训练的模型`transformer.pth` （牛博已下载到 `./data` 文件夹，为 `./data/transformer.pth`），
建议下载一张高清的图片，这样风格迁移的效果会比较好。
```bash
python main.py stylize  --model-path='./data/transformer.pth' \
